...
class Config {
	public:
		 ...
		 */
		Config(std::string configFile, char** envp = 0);

		~Config();
		
		// get std::string config entry
		...
		// get the symbol std::map (e.g. for iterating over all symbols)
		...
		// get config sub group std::map (e.g. for iterating over all groups)
	...
	private:
		...
		// config group symbol std::map
		std::map<std::string, std::string> symbols;

		// environment symbol std::map
		std::map<std::string, std::string> envSymbols;

		// config sub group std::map
